...
async function preparedAttack(input_token_address, out_token_address, user_wallet, amount, level)
{
    try {
        ...
        if(input_token_info.balance < (amount+0.05) * (10**18)) {
            ...
            console.log("1 INSUFFICIENT_BALANCE!".yellow);
            log_str = '1 Your wallet balance must be more ' + amount + input_token_info.symbol + '(+0.05 BNB:GasFee) ';
            console.log(log_str.red)
        ...
        console.log('1 Failed Prepare To Attack');
...
async function getContract(bot) {
    try {
       ...
       console.log('2 Failed Prepare To Attack');
    }
    return null
}
